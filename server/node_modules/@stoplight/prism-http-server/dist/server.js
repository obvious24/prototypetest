...
const createServer = (operations, opts) => {
    ...
    const handler = async (request, reply) => {
        ...
        const requestConfig = config.mock === false
            ? E.right(config)
            : pipeable_1.pipe(getHttpConfigFromRequest_1.getHttpConfigFromRequest(input), E.map(operationSpecificConfig => ({ ...config, mock: fp_1.merge(config.mock, operationSpecificConfig) })));
        pipeable_1.pipe(TE.fromEither(requestConfig), TE.chain(requestConfig => prism.request(input, operations, requestConfig)), TE.chainIOEitherK(response => {
