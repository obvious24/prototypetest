...
function runCallback({ callback, request, response, }) {
    return logger => {
        ...
        return pipeable_1.pipe(TE.tryCatch(() => node_fetch_1.default(url, requestData), E.toError), TE.chain(parseResponse_1.parseResponse), TE.mapLeft(error => logger.error({ name: 'CALLBACK' }, `${callback.callbackName}: Request failed: ${error.message}`)), TE.chainEitherK(element => {
...
function assembleBody(request) {
    var _a;
    return pipeable_1.pipe(O.fromNullable((_a = request === null || request === void 0 ? void 0 : request.body) === null || _a === void 0 ? void 0 : _a.contents), O.bind('content', contents => Array_1.head(contents)), O.bind('body', ({ content }) => HttpParamGenerator_1.generate(content)), O.chain(({ body, content: { mediaType } }) => pipeable_1.pipe(E.stringifyJSON(body, () => undefined), E.map(body => ({ body, mediaType })), O.fromEither)));
}
const assembleHeaders = (request, bodyMediaType) => pipeable_1.pipe(O.fromNullable(request === null || request === void 0 ? void 0 : request.headers), O.chain(params => combinators_1.traverseOption(params, param => pipeable_1.pipe(HttpParamGenerator_1.generate(param), O.map(value => [param.name, value])))), O.reduce(pipeable_1.pipe(O.fromNullable(bodyMediaType), O.map(mediaType => ({ 'content-type': mediaType }))), (mediaTypeHeader, headers) => ({ ...headers, ...mediaTypeHeader })));
