...
const eitherSequence = Apply_1.sequenceT(E.either);
const mock = ({ resource, input, config, }) => {
    const payloadGenerator = config.dynamic ? JSONSchema_1.generate : lodash_1.partial(JSONSchema_1.generateStatic, resource);
...
function runCallbacks({ resource, request, response, }) {
    return withLogger_1.default(logger => pipeable_1.pipe(O.fromNullable(resource.callbacks), O.map(callbacks => pipeable_1.pipe(callbacks, A.map(callback => callbacks_1.runCallback({ callback, request: parseBodyIfUrlEncoded(request, resource), response })(logger)())))));
}
function parseBodyIfUrlEncoded(request, resource) {
    ...
    if (!type_is_1.is(mediaType, ['application/x-www-form-urlencoded']))
        return request;
    const specs = pipeable_1.pipe(O.fromNullable(resource.request), O.chainNullableK(request => request.body), O.chainNullableK(body => body.contents), O.getOrElse(() => []));
...
const assembleResponse = (result, payloadGenerator) => logger => pipeable_1.pipe(E.Do, E.bind('negotiationResult', () => result), E.bind('mockedData', ({ negotiationResult }) => eitherSequence(computeBody(negotiationResult, payloadGenerator), computeMockedHeaders(negotiationResult.headers || [], payloadGenerator))), E.map(({ mockedData: [mockedBody, mockedHeaders], negotiationResult }) => {
    const response = {
