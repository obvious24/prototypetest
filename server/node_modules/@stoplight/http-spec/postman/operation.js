...
exports.transformPostmanCollectionOperations = (document) => {
    const collection = util_1.resolveCollection(document);
    const securitySchemes = securityScheme_1.transformSecuritySchemes(collection);
    let operations = [];
    collection.forEachItem(item => {
        operations = merge_1.mergeOperations(operations, [transformItem(item, securitySchemes)]);
    });
    return operations;
};
exports.transformPostmanCollectionOperation = ({ document, path, method, }) => {
    const collection = util_1.resolveCollection(document);
    const item = findItem(collection, method, path);
    if (!item) {
        throw new Error(`Unable to find "${method} ${path}"`);
    }
    return transformItem(item, securityScheme_1.transformSecuritySchemes(collection));
};
function transformItem(item, securitySchemes) {
    ...
    const request = request_1.transformRequest(item.request);
    const security = [];
    if (postmanSecurity) {
    ...
    const server = server_1.transformServer(item.request.url);
    return {
        ...
        summary: item.name,
        request,
        responses: item.responses.all().reduce((merged, response) => merge_1.mergeResponses(merged, [response_1.transformResponse(response)]), []),
        security,
...
function findPostmanSecurityScheme(auth, securitySchemes) {
    const securityScheme = securityScheme_1.transformSecurityScheme(auth, () => '1');
    if (!securityScheme)
        return;
    return securitySchemes.find(ss => securityScheme_1.isPostmanSecuritySchemeEqual(ss, securityScheme));
}
